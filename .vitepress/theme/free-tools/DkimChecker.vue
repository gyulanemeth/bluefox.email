...
function validateInputs() {
  ...
  if (shouldShowCaptcha()) {
    if (!captchaProbe.value)        return 'Please solve the captcha.'
...
async function checkDkim() {
  ...
  // Only block if probe is truly expired
  if (isProbeExpired()) {
  ...
  try {
    ...
    if (!res.ok || !(json.result && json.result.success)) {
      if (json.result && json.result.error && json.result.error.toLowerCase().includes('captcha')) {
        await refreshCaptcha()
        error.value = 'Captcha expired or invalid. Please solve the new captcha.'
        captchaSolved.value = false
        return
      } else {
        error.value = (json.result && json.result.error) ? json.result.error : 'Unknown error.'
        return
      }
    }
    ...
    captchaText.value   = ''
    captchaSolved.value = true // hide box for this probe's lifetime
  } catch (err) {
...
<template>
  <div class="dkim-checker">
    ...
    <div v-if="result" class="result-section">
      ...
      <div v-if="result.success"><p><strong>✓ DKIM Record Found</strong></p></div>
      <div v-else><p><strong>✗ DKIM Record Missing or Invalid</strong></p></div>
      <div class="info-section">
        ...
        <p><strong>DKIM Record:</strong> {{ result.rawRecord || result.record || 'Not found' }}</p>
      </div>
      <!-- DKIM Record Breakdown Table -->
        ...
        <table class="dkim-record-table">
          ...
          <tbody>
            <tr v-for="tag in dkimTags" :key="tag.tag">
              <td>{{ tag.tag }}</td>
              <td>
                <div class="dkim-key" v-if="tag.tag === 'p'">{{ tag.value }}</div>
...
@media (max-width: 768px) {
  ...
  .tool-form, .result-section {
    padding: 1rem;
    margin: 1rem 0;
  }
  .form-group input, .check-btn {
    padding: 0.75rem;
  }
}

/* --- Improved dark theme for DKIM Table --- */
