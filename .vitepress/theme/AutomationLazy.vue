...
// Lazy load the heavy Automation component only when needed
const Automation = defineAsyncComponent(() => import('./Automation.vue'))
...
onMounted(() => {
  ...
  const el = document.querySelector('#automation-heading')
  if (el) {
...
<template>
  <div>
    ...
    <div v-else style="min-height: 600px; display: flex; align-items: center; justify-content: center;">
      <div style="color: var(--vp-c-text-2);">Loading animation...</div>
    </div>
  </div>
</template>
