...
> **API reference.** For a narrative walkthrough with code samples of the legacy flat-URL contact endpoints, see [Contacts Management](/docs/api/contacts-management).
...
The flat `/v1/contacts/{projectId}` legacy shape (list/create/get/update/delete) behaves identically to the endpoints
above - see [Contacts Management](/docs/api/contacts-management) for its request/response details and code samples.
