...
faqs:
  ...
  - question: "How do I verify that a webhook request came from BlueFox Email?"
    answer: "BlueFox Email sends a secret key in the Authorization header as a Bearer token. In your endpoint, extract the key from the header and compare it to the secret key you configured in your project settings. Only process the request if the keys match."
  ...
  - question: "How do I rotate my secret key without losing webhook events?"
    answer: "Configure your endpoint to accept a list of valid secret keys rather than a single one. Check the incoming key against all keys in the list. This lets you add the new key and remove the old one without any downtime or missed events during the transition."
head:
  - - meta
    - name: description
...
Webhooks allow your application to receive real-time [notifications](/aws-concepts/ses-delivery-notifications) about email events such as opens, clicks, [bounces](/email-sending-concepts/bounces), [complaints](/email-sending-concepts/complaints), and subscriptions. By setting up a webhook, you can integrate email event data directly into your application for better tracking and automation.
