...
**Deliverability** describes what happens after that. The message might be placed in:
...
* message content;
* the quality and origin of the recipient list.
...
When you send a transactional email, triggered email or campaign, BlueFox processes the request and submits the message through the configured sending infrastructure.
...
A **hard bounce** usually indicates a permanent problem, such as an address that does not exist. Continuing to send to addresses that repeatedly fail can damage your reputation and make your list appear poorly maintained.

BlueFox processes bounce events from the sending infrastructure and records them in your email analytics. Addresses that should no longer receive emails can be added to a suppression list so that further sending attempts are prevented.

In addition, we may include bounce activity in digest emails, so you can stay informed about delivery issues without needing to monitor events in real time.

This protects both the recipient and your sending reputation.

Bounce processing does not eliminate the need to collect addresses carefully. It provides a safety mechanism after an address has already failed.

### Complaint processing

A complaint is typically generated when a recipient marks an email as spam.

Complaints are one of the clearest negative signals a sender can generate. A high complaint rate can indicate that recipients do not recognise the sender, did not expect the message or cannot find an easier way to stop receiving it.

BlueFox processes complaint notifications and includes them in the same digest reporting as bounce events. Affected recipients are added to a suppression list, preventing further sending where appropriate.

However, complaint processing happens after the damage has begun. The better approach is to avoid complaints by sending relevant emails only to people who expect them.
...
* **SPF**, which identifies servers authorised to send for a domain;
* **DKIM**, which adds a cryptographic signature to the message;
* **DMARC**, which connects domain authentication with reporting and domain-level policy.
...
It is important to note that dedicated IPs also introduce additional responsibility. Because reputation is no longer shared, your sending behaviour directly determines the reputation of that IP. This typically requires:
...
It is a must in BlueFox Email, not an option. It is required to obtain production access and to send emails from your own domain.
...
Transactional emails are associated with an action or an existing relationship. Examples include password resets, receipts, account notifications and service messages.
...
* email creation;
* transactional and triggered sending;
* campaigns;
...
* requesting and maintaining SES production access;
